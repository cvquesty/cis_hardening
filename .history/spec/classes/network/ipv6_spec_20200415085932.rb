...
describe 'cis_hardening::network::ipv6' do
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      it { is_expected.to contain_file_line('ipv6_accept_ra_all').with(
        'ensure' => 'present',
        'path'   => '/etc/sysctl.d/99-sysctl.conf',
        'line'   => 'net.ipv6.conf.all.accept_ra = 0',
      ).that_notifies('Exec[restart_ipv6_sysctl]')}

      it { is_expected.to contain_file_line('ipv6_acept_ra_default').with(
        'ensure' => 'present',
        'path'   => '/etc/sysctl.d/99-sysctl.conf',
      ...
      it { is_expected.to contain_file_line('ipv6_accept_redirects_all').with(
        'ensure' => 'present',
        'path'   => '/etc/sysctl.d/99-sysctl.conf',
        'line'   => 'net.ipv6.conf.all.accept_redirects = 0',
      ).that_notifies('Exec[restart_ipv6_sysctl]')}

      it { is_expected.to contain_file_line('ipv6_accept_redirects_default').with(
        'ensure' => 'present',
        'path'   => '/etc/sysctl.d/99-sysctl.conf',

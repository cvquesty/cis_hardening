...
class cis_hardening::setup::filesystem {
  ...
  exec { 'chkvartmp_noexec':
    ...
    onlyif  => "test ! 'mount | grep -E '\s/var/tmp\s' |grep noexec'",
  }
  ...
  exec { 'chkvartmp_nodev':
    ...
    onlyif  => "test ! 'mount | grep -E '\s/var/tmp\s' |grep nodev'",
  }
  ...
  exec { 'chkvartmp_nosuid':
    ...
    onlyif  => "test ! 'mount | grep -E '\s/var/tmp\s' |grep nosuid'",
  }
  ...
  exec { 'chkhome_nodev':
    ...
    onlyif  => "test ! 'mount |grep -E '\s/home\s' |grep nodev'",
  }
